...
@section('page-subtitle', 'Documents linked to your profile and reservations')
@section('content')
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4">
                    @if($doc->is_verified)
                        <span class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full"><i class="fas fa-check-circle mr-1"></i>Verified</span>
                    @elseif($doc->isExpired())
                        <span class="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full"><i class="fas fa-times-circle mr-1"></i>Expired</span>
                    @else
                        <span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full"><i class="fas fa-clock mr-1"></i>Pending Review</span>
                    @endif
                </td>
                <td class="px-6 py-4 text-xs text-gray-400">{{ $doc->expiry_date ? $doc->expiry_date->format('M d, Y') : '—' }}</td>
                <td class="px-6 py-4 text-xs text-gray-400">{{ $doc->created_at->format('M d, Y') }}</td>
                <td class="px-6 py-4 text-right">
                    <a href="{{ route('documents.download', $doc) }}" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium transition">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <form method="POST" action="{{ route('client.documents.store') }}" enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            ...
            <div>
                ...
                <select name="document_type" required class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                ...
                <select name="reservation_id" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    @foreach($reservations as $res)
                        <option value="{{ $res->id }}">{{ $res->property->title ?? 'Property #'.$res->id }} — {{ ucfirst($res->status) }}</option>
                    @endforeach
                </select>
            </div>
            @endif
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700 transition font-medium">
