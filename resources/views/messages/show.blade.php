...
@section('title', 'Chat - EstateFlow')
@section('page-title', $user->name)
@section('page-subtitle', ucfirst($user->role))

@section('content')
<div class="max-w-3xl flex flex-col" style="height: calc(100vh - 160px);">
        ...
        <div class="flex items-center gap-2 ml-auto">
            <div class="relative">
                <div class="w-8 h-8 rounded-full flex items-center justify-center font-semibold text-white text-sm
                    ...
                    {{ $user->role === 'contractor' ? 'bg-green-500'  : '' }}">
    ...
    <div class="flex-1 bg-white rounded-xl shadow-sm overflow-y-auto p-6 mb-4" id="chat-box">
            ...
            @forelse($messages as $msg)
                ...
                <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }}" data-msg-id="{{ $msg->id }}">
                    <div class="max-w-sm flex flex-col gap-1 {{ $isMine ? 'items-end' : 'items-start' }}">
                        @if($msg->reservation)
                            <div class="text-xs text-indigo-500 mb-1 {{ $isMine ? 'text-right' : 'text-left' }}">
        ...
        {{-- Typing indicator --}}
        <div id="typing-indicator" class="hidden flex justify-start mt-3">
        ...
        <form id="send-form" class="space-y-3" enctype="multipart/form-data">
            ...
            <select id="reservation-select" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs text-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                @foreach($reservations as $res)
                    <option value="{{ $res->id }}">
                        #{{ $res->id }} — {{ $res->property->title ?? 'Property' }}
                        @if(isset($res->client)) · {{ $res->client->full_name }} @endif
                        ({{ ucfirst($res->status) }})
                    </option>
                @endforeach
            </select>
            ...
            <div class="flex gap-2">
                ...
                <button type="submit"
                    class="bg-indigo-600 text-white px-5 py-2.5 rounded-xl hover:bg-indigo-700 transition self-end">
...
function buildBubble(msg) {
    ...
    if (msg.property) {
        ...
        prop.className = `text-xs text-indigo-500 mb-1 ${msg.mine ? 'text-right' : 'text-left'}`;
...
async function pollMessages() {
    try {
        ...
        if (msgs.length > 0) {
            ...
            const atBottom = document.getElementById('chat-box').scrollHeight - document.getElementById('chat-box').scrollTop <= document.getElementById('chat-box').clientHeight + 50;

            msgs.forEach(msg => {
                container.appendChild(buildBubble(msg));
                lastReceivedId = Math.max(lastReceivedId, msg.id);
            });
