        ...
        <form method="POST" action="{{ route('finance.payments.store') }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Reservation <span class="text-red-500">*</span></label>
                    <select name="reservation_id" id="reservation_id" onchange="fillClient(this)"
                ...
                <div>
                    ...
                    <select name="client_id" id="client_id"
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('client_id', $selectedReservation?->client_id) == $client->id ? 'selected' : '' }}>
                                {{ $client->full_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="payment_type" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        @foreach(['reservation','downpayment','equity','full_payment','other'] as $type)
                ...
                <div>
                    ...
                    <select name="payment_method" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        @foreach(['cash','bank_transfer','credit_card','gcash','check','pagibig'] as $method)
                            <option value="{{ $method }}" {{ old('payment_method', request('from_pagibig') ? 'pagibig' : '') === $method ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Payment Proof <span class="text-gray-400 font-normal">(receipt, screenshot)</span></label>
                ...
                @if(request('from_pagibig'))
                    <input type="hidden" name="from_pagibig" value="1">
                @endif
            </div>
            ...
            <div class="mt-6 flex gap-3">
                ...
                <a href="{{ route('finance.payments') }}" class="bg-gray-100 text-gray-600 px-6 py-2 rounded-lg text-sm hover:bg-gray-200 transition">Cancel</a>
            </div>
        </form>
    </div>
</div>
