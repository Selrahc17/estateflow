...
@section('title', 'Edit Lead — EstateFlow')
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('leads.update', $lead) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="assigned_agent_id" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        @foreach($agents as $agent)
                            <option value="{{ $agent->id }}" {{ old('assigned_agent_id', $lead->assigned_agent_id) == $agent->id ? 'selected' : '' }}>{{ $agent->full_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="interested_property_id" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property->id }}" {{ old('interested_property_id', $lead->interested_property_id) == $property->id ? 'selected' : '' }}>{{ $property->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Converted Client</label>
                    <select name="converted_client_id" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('converted_client_id', $lead->converted_client_id) == $client->id ? 'selected' : '' }}>{{ $client->full_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition">Update Lead</button>
