...
@section('title', 'Edit Agent - EstateFlow')
@section('page-title', 'Edit Agent')
@section('page-subtitle', $agent->full_name)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2 bg-indigo-50 border border-indigo-200 rounded-xl p-3 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-mono font-bold text-indigo-900">{{ $agent->agent_code }}</p>
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">First Name <span class="text-red-500">*</span></label>
                    <input type="text" name="first_name" value="{{ old('first_name', $agent->first_name) }}"
                ...
                <div>
                    ...
                    <input type="number" name="commission_rate" value="{{ old('commission_rate', $agent->commission_rate) }}" step="0.01" min="0" max="100"
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                    <textarea name="address" rows="2"
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('address', $agent->address) }}</textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="user_id" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ old('user_id', $agent->user_id) == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->email }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $agent->notes) }}</textarea>
                </div>
            </div>
            <div class="mt-6 flex gap-3">
                <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-indigo-700 transition font-medium">Update Agent</button>
                <a href="{{ route('agents.show', $agent) }}" class="bg-gray-100 text-gray-600 px-6 py-2 rounded-lg text-sm hover:bg-gray-200 transition">Cancel</a>
            </div>
        </form>
    </div>
</div>
